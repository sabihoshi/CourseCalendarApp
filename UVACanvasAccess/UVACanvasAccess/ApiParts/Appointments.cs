...
namespace UVACanvasAccess.ApiParts
{
    public partial class Api
    {
        ...
        public AppointmentGroupBuilder CreateAppointmentGroup(string title, IEnumerable<EventContext> contexts)
            => new AppointmentGroupBuilder(this, title, contexts);

        public AppointmentGroupBuilder CreateAppointmentGroup(string title, params EventContext[] contexts)
            => CreateAppointmentGroup(title, (IEnumerable<EventContext>) contexts);
        ...
        public AppointmentGroupBuilder EditAppointmentGroup(AppointmentGroup appointmentGroup)
        {
            ...
            {
                Logger.Warn(
                    "Editing an appointment group with no context codes and without supplying any. The operation " +
                    "might fail.");
            }
        ...
        public AppointmentGroupBuilder EditAppointmentGroup(AppointmentGroup appointmentGroup,
            IEnumerable<EventContext> contexts) => new AppointmentGroupBuilder(this, appointmentGroup.Id, contexts);

        public AppointmentGroupBuilder EditAppointmentGroup(AppointmentGroup appointmentGroup,
            params EventContext[] contexts)
            => EditAppointmentGroup(appointmentGroup, (IEnumerable<EventContext>) contexts);
        ...
        public async IAsyncEnumerable<AppointmentGroup> StreamAppointmentGroups(
            AppointmentVisibilityScope? scope = null,
            ...
            IEnumerable<EventContext> contexts = null)
        {
            ...
            if (contexts != null) args = args.Concat(contexts.Select(c => ("context_codes[]", c.ContextCode)));
            ...
            where T : IAppointmentGroupParticipant, IPrettyPrint
        {
            ...
            var response = await _client
                .GetAsync(
                    $"appointment_groups/{appointmentGroupId}/{(isUser ? "users" : "groups")}?registration_status=registered")
                .AssertSuccess();
        ...
        public async Task<AppointmentGroup> DeleteAppointmentGroup(ulong id, string reason = null)
        {
            var response = await _client.DeleteAsync($"appointment_groups/{id}" + (reason == null
                ? ""
                : $"?reason={reason}"));
            var model = JsonConvert.DeserializeObject<AppointmentGroupModel>(await response.AssertSuccess().Content
                .ReadAsStringAsync());
        ...
        internal async Task<AppointmentGroup> PostCreateAppointmentGroup(AppointmentGroupBuilder builder)
        {
            ...
            var model = JsonConvert.DeserializeObject<AppointmentGroupModel>(
        ...
        internal async Task<AppointmentGroup> PutUpdateAppointmentsGroup(AppointmentGroupBuilder builder)
        {
            var args = builder.Fields
                ...
                .Concat(builder.ArrayFields
                    .SelectMany(k => k, (k, v) => (k.Key, v)));
            var response = await _client.PutAsync($"appointment_groups/{builder.EditingId}",
