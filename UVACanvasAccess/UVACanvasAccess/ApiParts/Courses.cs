...
namespace UVACanvasAccess.ApiParts
{
    public partial class Api
    {
        ...
        {
            [ApiRepresentation("syllabus_body")] SyllabusBody = 1 << 0,
            [ApiRepresentation("term")] Term = 1 << 1,
            [ApiRepresentation("course_progress")] CourseProgress = 1 << 2,

            [ApiRepresentation("storage_quota_used_mb")]
            StorageQuotaUsedMb = 1 << 3,
            [ApiRepresentation("total_students")] TotalStudents = 1 << 4,
            [ApiRepresentation("teachers")] Teachers = 1 << 5,
            [ApiRepresentation("account_name")] AccountName = 1 << 6,
            [ApiRepresentation("concluded")] Concluded = 1 << 7,
            [ApiRepresentation("all_courses")] AllCourses = 1 << 8,
            [ApiRepresentation("permissions")] Permissions = 1 << 9,
            [ApiRepresentation("observed_users")] ObservedUsers = 1 << 10,
            [ApiRepresentation("course_image")] CourseImage = 1 << 11,
            ...
            [ApiRepresentation("total_scores")] TotalScores = 1 << 14,

            [ApiRepresentation("current_grading_period_scores")]
            CurrentGradingPeriodScores = 1 << 15,
            [ApiRepresentation("account")] Account = 1 << 16,
            [ApiRepresentation("sections")] Sections = 1 << 17,
            [ApiRepresentation("passback_status")] PassbackStatus = 1 << 18,
            [ApiRepresentation("favorites")] Favorites = 1 << 19,
        ...
        public CourseBuilder CreateCourse(ulong? accountId = null) => new CourseBuilder(this, false, accountId);
        ...
        public CourseBuilder EditCourse(ulong courseId, ulong? accountId = null)
            => new CourseBuilder(this, true, accountId, courseId);
        ...
        {
            var response = await RawGetCourse(accountId?.ToString() ?? "self", courseId.ToString(), includes,
                teacherLimit);

            var model = JsonConvert.DeserializeObject<CourseModel>(await response.AssertSuccess().Content
                .ReadAsStringAsync());
        ...
        private Task<HttpResponseMessage> RawDeleteCourse(string id, [NotNull] string action)
            => _client.DeleteAsync($"courses/{id}" + BuildQueryString(("event", action)));
