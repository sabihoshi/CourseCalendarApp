...
namespace UVACanvasAccess.ApiParts
{
    ...
    {
        public async Task<IEnumerable<Role>> ListRoles(RoleState states = 0, bool? showInherited = null,
        ...
        public Task<Role> CreateRole(string label,
            ...
            ulong? accountId = null)
            => CreateRole(label, permissions?.GetAsParams() ?? Enumerable.Empty<(string, string)>(), baseRoleType,
                accountId);

        private Task<HttpResponseMessage> RawCreateRole(string accountId, HttpContent content)
            => _client.PostAsync($"accounts/{accountId}/roles", content);
