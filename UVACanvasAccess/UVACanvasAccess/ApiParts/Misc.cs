...
namespace UVACanvasAccess.ApiParts
{
    public partial class Api
    {
        ...
        /// <param name="scopes">
        ///     The scope, and optionally subscopes, under which the data is stored. Omit scope to delete all
        ///     custom data.
        /// </param>
        ...
        private Task<HttpResponseMessage> RawDeleteCustomJson(string userId, string scopes, string ns)
            => _client.DeleteAsync($"users/{userId}/custom_data/{scopes}" + BuildQueryString(("ns", ns)));

        [PaginatedResponse]
        private Task<HttpResponseMessage> RawGetActivityStream(bool? onlyActiveCourses) => _client.GetAsync(
        ...
        private Task<HttpResponseMessage> RawLoadCustomJson(string userId, string scopes, string ns)
            => _client.GetAsync($"users/{userId}/custom_data/{scopes}" + BuildQueryString(("ns", ns)));

        private Task<HttpResponseMessage> RawStoreCustomJson(string userId, string scopes, HttpContent content)
            => _client.PutAsync($"users/{userId}/custom_data/{scopes}", content);
    }
}