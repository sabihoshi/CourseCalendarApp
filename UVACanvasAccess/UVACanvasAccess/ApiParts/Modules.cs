...
namespace UVACanvasAccess.ApiParts
{
    public partial class Api
    {
            ...
            ModuleIncludeType includes = ModuleIncludeType.None)
        {
            ...
            var response
                = await _client.GetAsync($"courses/{courseId}/modules" + BuildDuplicateKeyQueryString(args.ToArray()));
            ...
            ulong? studentId = null)
        {
            ...
            var response = await _client.GetAsync($"courses/{courseId}/modules/{moduleId}/items" +
                BuildDuplicateKeyQueryString(args));
            ...
            ModuleIncludeType includes = ModuleIncludeType.None)
        {
            ...
            var response = await _client.GetAsync($"courses/{courseId}/modules/{moduleId}" +
                BuildDuplicateKeyQueryString(args.ToArray()));

            var model = JsonConvert.DeserializeObject<ModuleModel>(await response.AssertSuccess().Content
                .ReadAsStringAsync());
            ...
            ulong assetId)
        {
            ...
            var response
                = await _client.GetAsync(
                    $"courses/{courseId}/module_item_sequence" + BuildDuplicateKeyQueryString(args));
            var model = JsonConvert.DeserializeObject<ModuleItemSequenceModel>(await response.AssertSuccess().Content
                .ReadAsStringAsync());
