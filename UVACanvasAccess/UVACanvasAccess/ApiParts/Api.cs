...
namespace UVACanvasAccess.ApiParts
{
    ...
    /// </summary>
    /// <remarks>
    ///     API instances are intended to be be reused between calls, and should only be constructed once per token, per
    ///     thread. <br />
    ...
    public partial class Api : IDisposable
    {
        ...
        {
            [ApiRepresentation("asc")] Ascending,
            [ApiRepresentation("desc")] Descending
        }
        ...
        /// <param name="baseUrl">
        ///     The base url of the API server, ending with the version number.
        ...
        /// <remarks>
        ///     Certain endpoints, for example those relating to the activity stream and personal files, can only
        ///     be called on <c>self</c>. Masquerading makes it possible to bypass this restriction.
        /// </remarks>
        public void MasqueradeAs(ulong id) { _masquerade = id; }

        /// <summary>
        ///     Stop "acting as" any user.
        /// </summary>
        public void StopMasquerading() { _masquerade = null; }
        ...
        {
            if (_masquerade != null) json.Add("as_user_id", _masquerade.ToString());
        ...
        private MultipartFormDataContent BuildMultipartHttpArguments(
            [NotNull] IEnumerable<ValueTuple<string, string>> args)
        {
            ...
            if (_masquerade != null) content.Add(new StringContent(_masquerade.ToString()), "as_user_id");
        ...
        private string BuildDuplicateKeyQueryString([NotNull] params ValueTuple<string, string>[] args)
        {
            ...
            {
                if (val != null) entries.Add($"{HttpUtility.UrlEncode(key)}={HttpUtility.UrlEncode(val)}");
            }

            if (_masquerade != null) entries.Add($"as_user_id={_masquerade.ToString()}");
        ...
        private string BuildQueryString([NotNull] params ValueTuple<string, string>[] args)
        {
            ...
            if (_masquerade != null) query["as_user_id"] = _masquerade.ToString();
