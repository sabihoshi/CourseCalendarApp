...
namespace UVACanvasAccess.ApiParts
{
    public partial class Api
    {
        ...
        public CreateUserBuilder CreateUser(string accountId = "self") => new CreateUserBuilder(this, accountId);
            ...
            bool includeCourse = false)
        {
            ...
            var response = await _client.GetAsync($"users/{userId.IdOrSelf()}/missing_submissions" +
        ...
        {
            var args = includeUngradedQuizzes switch
            {
        ...
        {
            if (string.IsNullOrWhiteSpace(sis)) return default;
        ...
        private Task<HttpResponseMessage> RawCreateUser(string accountId, HttpContent content)
            => _client.PostAsync($"accounts/{accountId}/users", content);

        private Task<HttpResponseMessage> RawEditUser(string userId, HttpContent content)
            => _client.PutAsync("users/" + userId, content);
        ...
        private Task<HttpResponseMessage> RawGetListUsers(string searchTerm,
            ...
            string sort,
            string order)
            => _client.GetAsync("accounts/" + accountId + "/users" + BuildQueryString(
        ...
        private Task<HttpResponseMessage> RawGetUserDetails(string userId)
            => _client.GetAsync("users/" + userId + BuildQueryString());

        private Task<HttpResponseMessage> RawGetUserProfile(string userId)
            => _client.GetAsync("users/" + userId + "/profile");
    }
}