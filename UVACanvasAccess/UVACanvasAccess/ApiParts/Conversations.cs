...
namespace UVACanvasAccess.ApiParts
{
    public partial class Api
    {
        ...
        ///     (Optional) Attachment ids. Attachments must have been uploaded to the current user's
        ///     attachments folder.
        /// </param>
        ...
        /// <param name="filterIntersection">
        ///     (Optional) If true, the predicate in <paramref name="filter" /> is an AND instead of
        ///     an OR.
        /// </param>
            ...
            bool includeParticipantAvatars = false)
        {
            ...
            if (includeParticipantAvatars) args.Add(("include[]", "participant_avatars"));

            if (filter != null) args.AddRange(filter.Select(id => ("filter[]", id.AsString)));
        ...
        /// <param name="includedMessages">
        ///     (Optional) The set of past messages to send to recipients not already present in the
        ///     conversation.
        /// </param>
        /// <param name="attachmentIds">
        ///     (Optional) Attachment ids. Attachments must have been uploaded to the current user's
            ...
            bool? addJournalEntry = null)
        {
            ...
            if (specificRecipients != null) args.AddRange(specificRecipients.Select(r => ("recipients[]", r.AsString)));
            ...
            var response
                = await _client.PostAsync($"conversations/{conversationId}/add_message", BuildHttpArguments(args));
            try
            {
                return new Conversation(this,
                    JsonConvert.DeserializeObject<ConversationModel>(await response.Content.ReadAsStringAsync()));
            }
            catch (InvalidOperationException)
            {
                throw new DoesNotExistException(
                    $"Conversation with id {conversationId} does not exist or is not visible to the current user.");
            }
        }
        ...
        ///     If the conversation with the given id does not exist or is not visible to the
        ///     current user.
        /// </exception>
        /// <returns>The conversation.</returns>
        public async Task<DetailedConversation> GetConversation(ulong conversationId, bool markAsRead = false)
        {
            ...
            var response
                = await _client.GetAsync($"conversations/{conversationId}" +
                    BuildDuplicateKeyQueryString(args.ToArray()));

            try
            {
                return new DetailedConversation(this,
                    JsonConvert.DeserializeObject<DetailedConversationModel>(
