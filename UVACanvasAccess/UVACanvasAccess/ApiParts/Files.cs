...
namespace UVACanvasAccess.ApiParts
{
    public partial class Api
    {
        ...
        {
            [ApiRepresentation("user")] User = 1 << 0,
            [ApiRepresentation("usage_rights")] UsageRights = 1 << 1
        }
        ...
        public enum FileSort : byte
        {
            ...
            [ApiRepresentation("size")] Size,
            [ApiRepresentation("created_at")] CreatedAt,
            [ApiRepresentation("updated_at")] UpdatedAt,
            [ApiRepresentation("content_type")] ContentType,
            [ApiRepresentation("user")] User
        }
        ...
        public async IAsyncEnumerable<CanvasFile> StreamPersonalFiles(
            ...
            Order? order = null)
        {
            ...
            if (searchTerm != null) args.Add(("search_term", searchTerm));
            ...
            if (onlyIncludeFileNames) args.Add(("only[]", "names"));

            if (sortBy != null) args.Add(("sort", sortBy.GetApiRepresentation()));

            if (order != null) args.Add(("order", order.GetApiRepresentation()));
        ...
        public Task<(decimal, decimal)> GetPersonalQuotaMiB()
        {
            ...
            if (!q.IsFaulted) return q.ThenApply(t => ((decimal) t.Item1 / 1048576, (decimal) t.Item2 / 1048576));
        ...
        public async Task<CanvasFile> GetPersonalFile(ulong fileId, FileIncludes? includes = null)
        {
            var response = await _client.GetAsync($"files/{fileId}" + (includes == null
                ? ""
                : BuildDuplicateKeyQueryString(includes.GetFlagsApiRepresentations().Select(r => ("include[]", r))
                    .ToArray())));
            var model = JsonConvert.DeserializeObject<CanvasFileModel>(await response.Content.ReadAsStringAsync());
            return new CanvasFile(this, model);
        }
        ...
        /// <param name="filePath">
        ///     The URI of the file on the local system.
        ...
        public async Task<Folder> UpdatePersonalFolder(ulong folderId, string name)
        {
            // todo incomplete
        ...
        public async Task<IEnumerable<Folder>> ResolvePersonalFolderPath(params string[] parts)
        {
            ...
            var response = await _client.GetAsync("users/self/folders/by_path/" + path + BuildQueryString())
                ...
                .ThenApply(JToken.Parse)
                .ThenApply(jt => jt.CheckError());
            if (response.IsT1) throw new DoesNotExistException(response.AsT1);

            return response.AsT0.ToObject<IEnumerable<FolderModel>>()
                .Select(fm => new Folder(this, fm));
        }

        internal Task<byte[]> DownloadPersonalFile(CanvasFile cf) => _client.GetByteArrayAsync(cf.Url);
        ...
        ///     How to handle a duplicate filename. Can be <c>overwrite</c> or <c>rename</c>.
        ///     The default is <c>overwrite</c>. Not applicable in a context where files are not placed in folders.
        /// </param>
        /// <param name="contentType">
        ///     The MIME type of the file. If absent, it will be determined by calling
        ...
        {
            if (contentType == null) contentType = MimeMapping.GetMimeMapping(filePath);
            ...
            {
                model = JsonConvert.DeserializeObject<CanvasFileModel>(await secondPostResponse.Content
                    .ReadAsStringAsync());
            }
            else
            {
