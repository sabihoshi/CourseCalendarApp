...
namespace UVACanvasAccess.Structures.Calendar
{
    ...
    {
        [ApiRepresentation("section")] Section,
        [ApiRepresentation("category")] Category
    }
    ...
    public readonly struct EventContext : IPrettyPrint
    {
        ...
        internal string ContextCode => SubType == null
            ? $"{Type.GetApiRepresentation()}_{Id}"
            : $"{Type.GetApiRepresentation()}_{SubType.GetApiRepresentation()}_{Id}";

        public string ToPrettyString() => $"EventContext ({ContextCode}) {{" +
            ($"\n{nameof(Type)}: {Type}," +
