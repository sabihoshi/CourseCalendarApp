...
namespace UVACanvasAccess.Structures.Quizzes
{
    ...
    {
        [ApiRepresentation("practice_quiz")] PracticeQuiz,
        [ApiRepresentation("assignment")] Assignment,
        [ApiRepresentation("graded_server")] GradedSurvey,
        [ApiRepresentation("survey")] Survey
    }

    [PublicAPI]
    public enum ScoringPolicy : byte
    {
        [ApiRepresentation("keep_highest")] KeepHighest,
        [ApiRepresentation("keep_latest")] KeepLatest
    }

    [PublicAPI]
    public enum HideResults : byte
    {
        [ApiRepresentation("never")] Never,
        [ApiRepresentation("always")] Always,
    ...
    public class Quiz : IPrettyPrint
    {
        ...
        internal Quiz(Api api, QuizModel model)
        {
            ...
            HideResults = (model.HideResults ?? "never").ToApiRepresentedEnum<HideResults>()
                .Expect(
                    () => new BadApiStateException($"Quiz.HideResults was an unexpected value: {model.HideResults}"));
            ...
            ScoringPolicy = model.ScoringPolicy?.ToApiRepresentedEnum<ScoringPolicy>()
                .Expect(() => new BadApiStateException(
                    $"Quiz.ScoringPolicy was an unexpected value: {model.ScoringPolicy}"));
        ...
        [CanBeNull] public string AccessCode { get; }

        public string Description { get; }

        public string HtmlUrl { get; }

        [CanBeNull] public string IpFilter { get; }
        ...
        [CanBeNull] public string SpeedGraderUrl { get; }
        ...
        public string ToPrettyString() => "Quiz {" +
            ($"\n{nameof(Id)}: {Id}," +
                $"\n{nameof(Title)}: {Title}," +
