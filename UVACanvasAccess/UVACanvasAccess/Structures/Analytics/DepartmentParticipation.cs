...
namespace UVACanvasAccess.Structures.Analytics
{
    ...
    public readonly struct DepartmentParticipation : IPrettyPrint
    {
        ...
        public string ToPrettyString() => "DepartmentParticipation {" +
        ...
        public class DateEntry : IPrettyPrint
        {
            ...
            public string ToPrettyString() => "DateEntry {" +
                ($"\n{nameof(Views)}: {Views}," +
        ...
        public class Categories : IPrettyPrint
        {
            ...
            {
                var dict = entries.GroupBy(e => e.Category)
                    .ToDictionary(g => g.Key, g => g.ToList())
                    .ValSelect(l => l.First().Views);

                if (dict.TryGetValue("announcements", out var n)) Announcements = n;

                if (dict.TryGetValue("assignments", out n)) Assignments = n;

                if (dict.TryGetValue("collaborations", out n)) Collaborations = n;

                if (dict.TryGetValue("conferences", out n)) Conferences = n;

                if (dict.TryGetValue("discussions", out n)) Discussions = n;

                if (dict.TryGetValue("files", out n)) Files = n;

                if (dict.TryGetValue("general", out n)) General = n;

                if (dict.TryGetValue("grades", out n)) Grades = n;

                if (dict.TryGetValue("groups", out n)) Groups = n;

                if (dict.TryGetValue("modules", out n)) Modules = n;

                if (dict.TryGetValue("other", out n)) Other = n;

                if (dict.TryGetValue("pages", out n)) Pages = n;

                if (dict.TryGetValue("quizzes", out n)) Quizzes = n;
            }
            ...
            public string ToPrettyString() => "ByCategory {" +
