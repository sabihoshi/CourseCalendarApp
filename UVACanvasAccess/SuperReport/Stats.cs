...
namespace SuperReport
{
    ...
    {
        [PublicAPI] public ImmutableList<decimal> Data { get; }

        [PublicAPI] public decimal Mean { get; }

        [PublicAPI] public decimal Q1 { get; }

        [PublicAPI] public decimal Median { get; }

        [PublicAPI] public decimal Q3 { get; }

        [PublicAPI] public decimal Mode { get; }

        [PublicAPI] public double Sigma { get; }
        ...
        public Stats(IEnumerable<decimal> data)
        {
            ...
            var mean = Mean;
            Sigma = Math.Sqrt(Data.Aggregate(0.0, (acc, s) => acc + Math.Pow(Convert.ToDouble(s - mean), 2)) /
                Data.Count);
        }
    }
}