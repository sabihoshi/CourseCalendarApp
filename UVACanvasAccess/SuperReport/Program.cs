...
namespace SuperReport
{
    internal static class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            Console.WriteLine(
                $"TAKING {(coursesPerTeacher == default ? "ALL" : coursesPerTeacher.ToString())} courses per teacher.");
            Console.WriteLine(
                $"TAKING {(assignmentsPerCourse == default ? "ALL" : assignmentsPerCourse.ToString())} assignments per course.");
            Console.WriteLine(
                $"TAKING {(submissionsPerAssignment == default ? "ALL" : submissionsPerAssignment.ToString())} submissions per assignment.");
            ...
            if (sampleTake != default) sample = sample.Take(sampleTake);
            ...
            await foreach (var user in sample)
            {
                try
                {
                    if (await user.IsTeacher())
                    {
                        ...
                        if (coursesPerTeacher != default) enrollmentsStream = enrollmentsStream.Take(coursesPerTeacher);
                        ...
                        await foreach (var (enrollment, course) in enrollmentsStream)
                        {
                            ...
                            if (!overallCoursePerformanceObj.ContainsKey(course.Id.ToString()))
                            {
                                ...
                                await foreach (var studentEnrollment in studentEnrollments)
                                {
                                    ...
                                    courseScores.Add(string.IsNullOrEmpty(currentScore)
                                        ? 0
                                        : Convert.ToDecimal(grades.CurrentScore));
                                }

                                if (!courseScores.Any()) continue;
                            ...
                            if (assignmentsPerCourse != default) assignments = assignments.Take(assignmentsPerCourse);
                            ...
                            foreach (var assignment in assignments)
                            {
                                ...
                                foreach (var submission in allSubmissions)
                                {
                                    ...
                                    submissionsObj[
                                            $"c_{course.Id}|a_{assignment.Id}|u_{submitter.Id}|n_{submission.Attempt ?? 0}"]
                                        = new JObject
                                        {
                                ...
                                foreach (var submission in submissions)
                                {
                                    var submitter = await api.GetUser(submission.UserId);
                                    Debug.Assert(
                                        !assignmentsIndividualObj.ContainsKey(
                                            $"c_{course.Id}|a_{assignment.Id}|u_{submitter.Id}"));
                                    ...
                                    {
                                        minutesSubmittedBeforeDueDate
                                            = assignment.DueAt.Value - submission.SubmittedAt.Value;
                                    }

                                    assignmentsIndividualObj[$"c_{course.Id}|a_{assignment.Id}|u_{submitter.Id}"]
                                        = new JObject
                                        {
                                            ["assignmentId"]   = assignment.Id,
                                            ["courseId"]       = course.Id,
                                            ...
                                            ["minutesSubmittedBeforeDueDate"]
                                                = minutesSubmittedBeforeDueDate?.TotalMinutes
                                        };
                                }
                            }
                        }
