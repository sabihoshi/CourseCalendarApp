    ...
    internal static class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                {
                    if (ignoreIds.Contains(taughtCourse.CourseId)) continue;
                            ...
                            assignment.Id))
                        {
                            if (sections != null)
                            {
                                ...
                                var ok = await api.StreamUserEnrollments(submission.UserId)
                                    ...
                                    .AnyAsync(e => e.CourseSectionId != null &&
                                        sections.Any(s => s.Id == e.CourseSectionId));
                                if (!ok) continue;
                            }
                            ...
                            {
                                Console.WriteLine(
                                    $"Ungraded Assignment: {assignment.Name} ({assignment.Id}) submitted by user {submission.UserId} " +
